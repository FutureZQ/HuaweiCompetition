...
//交叉基因与变异，p_rato为变异率
void crossGen(vector<int> &ind1, vector<int> &ind2,double p_rato)
{
	...
	for (int i = min_index; i <= max_index; i++)
	{
		...
		int temp = ind1[i];
		double r = (double)rand() / RAND_MAX;//判断是否变异
		if (r <= p_rato)
		{
			ind1[i] = !ind2[i];
		}
...
//初始化种群
vector<vector<int>> generateNewIndividuals(Graph *pG,int number)
{

	srand((unsigned)time(NULL));//设置随机种子
	int node_number = pG->mVexNumNet;
	vector<vector<int>> individuals(0,vector<int>(node_number));
	double radio = 0.25;	//服务器节点的比例	
...
void genAlgorithm(Graph *pG,int individual_number)
{
	...
	//初始化种群
	vector<vector<int>> individual_list = generateNewIndividuals(pG, individual_number);//产生individual_number个个体作为一个种群
	...
	while (count-- >0)
	{
		...
		individual_list._Pop_back_n(individual_number);//弹出之前的个体

		///////////////////////// 交换基因（变异）////////////////////////////////////////////////
		for (int i = 0; i < individual_number / 2; i++)//交换基因
		{
			crossGen(individual_list[2 * i], individual_list[2 * i + 1],0.0005);
		}
